...
enum {BLACKID=(256-6), REDID, GREENID, BLUEID, WHITEID, GRAYID};
...
void initGame() {
 ...
 initBarriers();
 initBalls();
 unsigned short colors[6] = {0, ((31) | (0)<<5 | (0)<<10), ((0) | (31)<<5 | (0)<<10), ((0) | (0)<<5 | (31)<<10), ((31) | (31)<<5 | (31)<<10), ((15) | (15)<<5 | (15)<<10)};
 DMANow(3, enemyPal, ((unsigned short *)0x5000000), 256);
 for (int i = 0; i < 6; i++) {
...
void initBullets() {
 for (int i = 0; i < 5; i++) {
  ...
  playerbullets[i].color = ((31) | (31)<<5 | (31)<<10);
  playerbullets[i].active = 0;
 }
 for (int i = 0; i< 5 * 3; i++) {
  ...
  ballbullets[i].color = ((31) | (31)<<5 | (31)<<10);
  ballbullets[i].active = 0;
 }
}
...
void drawBullet(BULLET* b) {
 if(b->active) {
  drawRect4(b->oldCol, b->oldRow, b->width, b->height, 0);
...
void updateBall(BALL* b) {
 ...
 for (int i = 0; i < 15; i++) {
  ...
  } else if (i >= 15 / 3 && i < (15 / 3) * 2) {
   if (balls[i].active) {
    secondRowCleared = 0;
   }
  } else if (i >= (15 / 3) * 2 && i < 15) {
   if (balls[i].active) {
    thirdRowCleared = 0;
   }
  }
